...
impl UnlockDb {
    ...
    pub fn parse(content: &str) -> Result<Self> {
        ...
        for line in content.lines() {
            ...
            if parts.len() >= 3 {
                let song_id = parts[0].trim().to_string();
    ...
    pub fn save<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        ...
        for (song_id, (unlock_type, unlocks)) in &self.entries {
            lines.push(format!("{},{},{}", song_id, unlock_type, unlocks));
        }
    ...
    pub fn get(&self, song_id: &str) -> Option<(i32, i32)> {
    ...
    pub fn update(&mut self, song_id: &str, unlock_type: i32, unlocks: i32) {
        self.entries
            .insert(song_id.to_string(), (unlock_type, unlocks));
    }

    /// Update from UnlockData
    pub fn update_from_data(&mut self, song_id: &str, data: &UnlockData) {
    ...
    pub fn has_unlock_type_changed(&self, song_id: &str, new_type: i32) -> bool {
    ...
    pub fn has_unlocks_changed(&self, song_id: &str, new_unlocks: i32) -> bool {
    ...
    /// Get all entries
    pub fn iter(&self) -> impl Iterator<Item = (&String, &(i32, i32))> {
        self.entries.iter()
    }
...
03000,3,255"#;
        ...
        assert_eq!(db.get("01000"), Some((1, 31)));
        assert_eq!(db.get("02000"), Some((2, 15)));
        assert_eq!(db.get("03000"), Some((3, 255)));
    ...
    fn test_update_and_save() {
        ...
        db.update("01000", 1, 31);
        db.update("02000", 2, 15);

        assert_eq!(db.len(), 2);
        assert!(db.contains("01000"));
    ...
    fn test_change_detection() {
        ...
        assert!(db.has_unlock_type_changed("01000", 2));
        assert!(!db.has_unlock_type_changed("01000", 1));
        assert!(db.has_unlocks_changed("01000", 63));
        assert!(!db.has_unlocks_changed("01000", 31));
    }
}
